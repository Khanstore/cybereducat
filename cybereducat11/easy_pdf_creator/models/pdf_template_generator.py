...
class PdfTemplateGenerator(models.Model):
	...
	def print_template(self, ids):
		...
		output = BytesIO(pdfkit.from_string(html,False,options=options, css=[path,path2])) 
		out = base64.encodestring(output.getvalue())
	...
	def _set_function_horizontal_pattern(self, text, ids):
		match = re.findall(r'{function_horizontal:[\w.-]+}', text)
		obj = self.env[ self.model_id.model ].sudo().search([('id','=',ids)])
		array_match = []
		for item in match:
			s_item = item.replace('{','')
		...
		for patern_name in array_match:
			...
			data =  getattr(model, method_name)(ids)
			if data:
				table_lines = '<table style="border: 1px solid #dddddd;width:100%;border-collapse: collapse; ">'
				...
				for line in data['data']:
					row = '<tr>'
					for td in line:
						row += '<td style="border: 1px solid #dddddd;padding:1px;" class="o2m_cell">'+ td +'</td>'
					table_lines += row +'</tr>'
				table_lines += '</table>'
	...
	def _set_function_vertical_pattern(self, text, ids):
		match = re.findall(r'{function_vertical:[\w.-]+}', text)
		obj = self.env[ self.model_id.model ].sudo().search([('id','=',ids)])
		array_match = []
		for item in match:
			s_item = item.replace('{','')
		...
		for patern_name in array_match:
			...
			if data:
				...
				i = 0
				for th in headers:
					new_row = '<tr><td style="border: 1px solid #dddddd;padding:1px;" class="o2m_header">'+th+'</td>'
					for td in datas:
						if i < len(td):
							# print '===========td', i, td[i]
							new_row += '<td style="border: 1px solid #dddddd;padding:1px;" class="o2m_cell">'+ td[i] +'</td>'
					new_row += '</tr>'
					table_lines += new_row
					i += 1
				table_lines += '</table>'
